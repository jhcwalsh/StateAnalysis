...
STRATEGIES = ["PIT_MaxSharpe", "PIT_MinVar", "ProbWeighted_MaxSharpe", "Oracle_MaxSharpe", "Static_6040", "EqualWeight",
              "PIT_LongOnly_MaxSharpe", "PIT_RiskParity", "Oracle_LongOnly_MaxSharpe"]   # last three added 2026-09-05
EXPOST = ["InSample_MaxSharpe_expost", "InSample_LongOnly_expost"]   # full-sample moments + smoothed labels; measure look-ahead
def longonly_weights(mu: Series, Sigma: DataFrame, rf=0.0) -> (Series, dict)
# max Sharpe s.t. w >= 0, sum w = 1 (SLSQP from equal weights); flags{converged, rank_deficient}
def risk_parity_weights(Sigma: DataFrame, tol=1e-8, max_iter=10_000) -> (Series, dict)
# equal risk contribution, long-only, fully invested (Maillard, Roncalli, Teiletche 2010); flags{converged, rank_deficient}
...
#                 insample_fallback, pit_longonly_fallback, pit_riskparity_fallback, oracle_longonly_fallback,
#                 insample_longonly_fallback, longonly_nonconverged, riskparity_nonconverged, negsum, rank_deficient),
#                 params (dict: start, min_regime_obs, cost_bp, leverage_cap)
# There is no separate insample_sharpe(): the in-sample number is the InSample_MaxSharpe_expost row of perf.
def lookahead_decomposition(perf: DataFrame, family="unconstrained") -> dict
# reads the InSample_MaxSharpe_expost / Oracle_MaxSharpe / PIT_MaxSharpe rows of one perf table (family="longonly":
# InSample_LongOnly_expost / Oracle_LongOnly_MaxSharpe / PIT_LongOnly_MaxSharpe, published as assets.lookahead_longonly);
...
- [ ] `mv_weights`: the notebook's optimiser with its guards (pseudo-inverse; negative-sum and rank-deficiency counted, never silenced; gross-leverage cap 3.0). Unconstrained long-short is kept for continuity of the look-ahead decomposition. Added 2026-09-05: `longonly_weights` (no shorts, fully invested, SLSQP) and `risk_parity_weights` (equal risk contribution on the regime covariance), giving `PIT_LongOnly_MaxSharpe`, `PIT_RiskParity`, and the long-only comparators `Oracle_LongOnly_MaxSharpe` / `InSample_LongOnly_expost` so the look-ahead decomposition is also measured under the long-only constraint (`assets.lookahead_longonly`; report-only acceptance rows `pit_longonly_sharpe`, `pit_riskparity_sharpe`, `longonly_moment_lookahead`, `longonly_label_lookahead`).
- [ ] `backtest`: decision at the end of month d using labels available by then (label d-1 under a 1-month lag), weights earn month d+1, expanding-window regime moments from returns <= d aligned by `available_at`. Strategies: `PIT_MaxSharpe`, `PIT_MinVar`, `ProbWeighted_MaxSharpe` (mixture moments on the walk-forward filtered probabilities; the §1 purpose-3 strategy), `Oracle_MaxSharpe` (full-sample smoothed labels: label look-ahead), `Static_6040`, `EqualWeight`, `PIT_LongOnly_MaxSharpe`, `PIT_RiskParity`, `Oracle_LongOnly_MaxSharpe`, plus the ex-post comparators `InSample_MaxSharpe_expost` and `InSample_LongOnly_expost` (`include_expost`). Fallback to 60/40 when the current regime has fewer than `min_regime_obs = 15` paired months, and for the constrained strategies also when the optimiser does not converge (counted). Start 2010-01. Turnover tracked; `cost_bp` applied per unit turnover and reported at 0 and 10 bp.
- [ ] Look-ahead decomposition as the notebook's Cell L: in-sample (full-sample moments, smoothed labels) -> oracle -> PIT; `moment_lookahead`, `label_lookahead`, `total`. Reported in `summary.json["assets"]["lookahead"]`, and for `family="longonly"` in `summary.json["assets"]["lookahead_longonly"]`; report-only rows in the acceptance table are `pit_sharpe`, `oracle_sharpe`, `insample_sharpe`, `label_lookahead`, `moment_lookahead`, `growth_share_6040`, `backtest_placebo_pct`, `pit_longonly_sharpe`, `pit_riskparity_sharpe`, `longonly_moment_lookahead`, `longonly_label_lookahead`; no thresholds.
- [ ] `backtest_placebo`: 200 label shuffles (each reruns the backtest), percentile of the real PIT Sharpe.
- [ ] Outputs: `output/backtest_returns.csv`, `output/portfolio_weights.csv` (PIT_MaxSharpe, ProbWeighted, PIT_LongOnly_MaxSharpe and PIT_RiskParity weights by month); figures fig 10 wealth curves (log) with drawdown panel, fig 11 PIT max-Sharpe weights over time.
...
- [x] Deployment checklist in §12 completed on the Mini. (deployment checklist executed on the Mini 2026-09-04: cloned to ~/apps/states, image built, pinned run published on the volume, Streamlit on localhost:8505 beside regimes:8503, terrarium:8502, test:8501; Cloudflare published application route `states.lazyeconomist.com` -> HTTP localhost:8505 added on the lazyeconomist.com tunnel; https://states.lazyeconomist.com answers 200. Port 8505 confirmed. Refresh with vintage 2026-08 executed inside the container (equivalent to the button) after the vintage fixes: published, acceptance passed, asset stage published, current month 2026-07 Overheating; 8505 = states recorded in the MacMiniHosting port table), the tunnel route still has to be published in Cloudflare Zero Trust)
...
- 2026-09-05 — Two site documents added as pages of the States app: a plain-language Introduction and a Methodology paper with references, both written as Markdown under `docs/site/` and rendered by `regime_v2/regime_v2/sitedocs.py` against the published run, so every number they state is the number the dashboard shows (contract: `docs/site/CONTRACT.md`). The engine now draws seven documentation figures on every run (`docfigs.py`) and stores the placebo null draws in `summary.json` for the placebo histograms. Reason: the documents must not drift from the app; a hand-typed number would be wrong after the first refresh. The published transition matrix and expected durations are the full-sample fit; mean run lengths and every headline number are from the walk-forward labels, and the paper says so. Streamlit's automatic sidebar nav (which labels the entrypoint "app") is hidden in favour of a masthead nav row on every page.
- 2026-09-05 — Dashboard opening screen rebuilt around the latest assessment: the θ explorer moved into the first Results tab (it still defines the θ the Factor gaps and State space tabs draw), and the walk-forward probability chart took its place on the opening screen, above a trace of the numbers the published label was read from — each gap against the ±θ Schmitt trigger with the side a held sign carries forward, the factor levels behind the gaps, the vintage and publication lag, this month's and the previous month's filtered probabilities, and the transition-matrix persistence of the current state. The trace is drawn from the published full-sample-loading gaps while the labels come from the unpublished walk-forward gaps, so it always states that provenance and says explicitly when the two disagree. Also fixed the masthead's link back to lazyeconomist.com, which had never been clickable on any page: Streamlit's transparent header is a full-width bar over the top 60px of the page and swallowed the click, so `stHeader` and `stToolbar` now pass pointer events through (the status widget keeps its own), and the anchor carries `target="_self"` because Streamlit rewrites a target-less anchor to `_blank`.
- 2026-09-05 — Long-only max-Sharpe and risk-parity strategies added to the asset stage, with long-only oracle and in-sample comparators so the look-ahead decomposition is measured under the constraint rather than asserted (the paper's Limitations had claimed a constrained optimiser "would almost certainly show smaller look-ahead"). The unconstrained strategies and their numbers are unchanged; the placebo still shuffles against `PIT_MaxSharpe` only. Documents report the constrained decomposition from placeholders without typing a direction.
...
- **Refresh:** `python run.py --vintage YYYY-MM` downloads the vintage, rebuilds `output/` and `figs/`, and re-runs the acceptance tests. A refresh that fails any §8 test must not replace the previously published outputs; it exits non-zero and leaves the last good `output/` in place. On the Mini this runs unattended: `scripts/refresh_states.sh` (LaunchAgent `deploy/com.lazyeconomist.states.refresh.plist`, 07:00 local on the 10th of each month, previous month's vintage) and pushes an ntfy alert with the log tail on any non-zero exit (decided 2026-09-05; the topic lives in the untracked `~/apps/states/.refresh.env`).
- **Published surface:** the dashboard (`app.py`) reading `output/`, the two document pages (`pages/1_Introduction.py`, `pages/2_Methodology.py`) rendering `docs/site/introduction.md` and `docs/site/methodology.md` with every number substituted from the published `summary.json` and every figure served from `figs/` under the contract in `docs/site/CONTRACT.md` (each page also offers a self-contained HTML download that prints to PDF), the seven documentation figures (`doc_pipeline`, `doc_quadrants`, `doc_timing`, `doc_lookahead`, `doc_placebo`, `doc_loadings`, `doc_transition`, drawn by `regime_v2/regime_v2/docfigs.py` on every run), plus the seven engine figures, the four asset-stage figures (`fig8_regime_returns.png`, `fig9_mixture_6040.png`, `fig10_backtest_wealth.png`, `fig11_pit_weights.png`), `regime_labels.csv`, and the four asset-stage CSVs (`regime_returns.csv`, `regime_corr_<Regime>.csv`, `backtest_returns.csv`, `portfolio_weights.csv`) as downloadable files. Nothing on the site is produced by the notebook.
